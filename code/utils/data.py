...
class SynthesizerDataset(Dataset):
    ...
    def compute_normalization(self):
        ...
        for n in range(len(self.spectral_files)):
            data = np.load(self.spectral_files[n])
    ...
    def analyze_dataset(self):
        ...
        loaded = np.load(self.data_files[0])
        self.param_names = list(loaded['param'].item().keys())
        # Keep some reference parameter values
        self.param_values = [loaded['param'].item()[v] for v in self.param_names]
        data = np.load(self.spectral_files[0])
        ...
        for n in range(len(self.data_files)):
            file = np.load(self.data_files[n]) 
        ...
        self.final_params = [self.param_names[i] for i in self.params_idx]
        self.final_std = [self.params_std[i] for i in self.params_idx]
    ...
    def __getitem__(self, idx):
        ...
        for d in self.data:
            ...
            if (os.path.exists(self.data_files[idx] + '_' + d + '.npy')):
                data = np.load(self.data_files[idx] + '_' + d + '.npy')
            else:
                if (d == 'mel'):
...
class SpecData(Dataset):
    """Single data type"""
    def __init__(self, spectral_files, transform=None, data_type='mel', stats=None, set_type=None):
...
class CompSynthesizerDataset(Dataset):
    ...
    """
    def __init__(self, datadir, transform=None, data='mel_mfcc', splits=[.8, .1, .1], shuffle_files=True):
        self.data = data.split("_") #["mel", "mfcc"] or ["mel"]
        # Metadata and raw
        self.data_files = []
        # Transform files
        self.spectral_files = {}
        ...
        for dtype in self.data:
            self.spectral_files[dtype] = sorted(glob.glob(datadir + '/' + dtype + '/*.npy'))
            self.trans_datasets[dtype] = SpecData(self.spectral_files[dtype], data_type=dtype, transform=transform)
    ...
    def create_splits(self, splits, shuffle_files):
        ...
        self.test_trans = {}
        for dtype in self.data:
            self.valid_trans[dtype] = SpecData([self.spectral_files[dtype][i] for i in valid_idx], data_type=dtype, stats=[self.means[dtype],self.vars[dtype]], set_type="valid")
            self.test_trans[dtype] = SpecData([self.spectral_files[dtype][i] for i in test_idx], data_type=dtype, stats=[self.means[dtype],self.vars[dtype]], set_type="test")
            self.trans_datasets[dtype] = SpecData([self.spectral_files[dtype][i] for i in train_idx], data_type=dtype, stats=[self.means[dtype],self.vars[dtype]], set_type="train")
...
def load_dataset(args, **kwargs):
    if (args.dataset in ['toy'], ["32par"]):
        dset_train = CompSynthesizerDataset(args.path + '/' + args.dataset, data=args.data, **kwargs)
    ...
    train_loader = DataLoader(dset_train, batch_size=args.batch_size, shuffle=True, num_workers=args.nbworkers, pin_memory=True, **kwargs)
    valid_loader = DataLoader(dset_valid, batch_size=args.batch_size, shuffle=(args.train_type == 'random'), num_workers=args.nbworkers, pin_memory=True, **kwargs)
    test_loader = DataLoader(dset_test, batch_size=args.batch_size, shuffle=(args.train_type == 'random'), num_workers=args.nbworkers, pin_memory=True, **kwargs)
